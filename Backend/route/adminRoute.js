...
import { propertyModel,userModel,buyerModel,contactUsModel } from '../model/table.js'
...
adminRoute.post('/add-contact-us',async(req,res)=>{
  const {name, email, contact, message,subject} = req.body;
  const data = new contactUsModel({name, email, contact, message,subject});
  const result = await data.save();
  if(result){
    res.json({
      code:200,
...
adminRoute.get('/get-contact-us',async(req,res)=>{
